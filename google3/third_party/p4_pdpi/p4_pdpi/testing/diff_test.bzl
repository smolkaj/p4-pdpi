...
file, run:
```sh
    blaze run <diff test target> -- --update`
```
...
def _diff_test_impl(ctx):
    ...
    When invoked as `blaze run <target> -- --update`, will update the `expected`
    file to match the contents of the `actual` file. Note that the file must
    already exist.
    """

    # Write test script that will be executed by 'blaze test'.
    ctx.actions.write(
...
diff_test = rule(
    ...
    To update the expected file, run `blaze run <target> -- --update`.
    """,
...
To auto-generate or update, run `blaze run <target> -- --update`.
""",
            mandatory = True,
            allow_single_file = True,
        ),
...
def cmd_diff_test(name, actual_cmd, expected, tools = [], visibility = None):
