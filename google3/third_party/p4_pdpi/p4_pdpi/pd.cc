...
#include "third_party/p4_pdpi/p4_pdpi/pd.h"
...
#include "third_party/p4_pdpi/gutil/collections.h"
#include "third_party/p4_pdpi/gutil/proto.h"
#include "third_party/p4_pdpi/gutil/status.h"
#include "third_party/p4_pdpi/p4_pdpi/internal/ordered_protobuf_map.h"
#include "third_party/p4_pdpi/p4_pdpi/ir.h"
#include "third_party/p4_pdpi/p4_pdpi/ir.proto.h"
#include "third_party/p4_pdpi/p4_pdpi/utils/ir.h"
#include "third_party/p4_pdpi/p4_pdpi/utils/pd.h"
...
absl::StatusOr<const proto2::FieldDescriptor *> GetFieldDescriptor(
    const proto2::Message &parent_message, const std::string &fieldname) {
...
absl::StatusOr<proto2::Message *> GetMutableMessage(
    proto2::Message *parent_message, const std::string &fieldname) {
...
absl::StatusOr<const proto2::Message *> GetMessageField(
    const proto2::Message &parent_message, const std::string &fieldname) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(parent_message, fieldname));
  if (field_descriptor == nullptr) {
...
absl::StatusOr<const proto2::Message *> GetRepeatedMessage(
    const proto2::Message &parent_message, const std::string &fieldname,
    int index) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(parent_message, fieldname));
...
absl::StatusOr<proto2::Message *> AddRepeatedMutableMessage(
    proto2::Message *parent_message, const std::string &fieldname) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(*parent_message, fieldname));
  if (field_descriptor == nullptr) {
...
absl::StatusOr<bool> GetBoolField(const proto2::Message &message,
...
absl::StatusOr<int32_t> GetInt32Field(const proto2::Message &message,
                                      const std::string &fieldname) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(message, fieldname));
  RETURN_IF_ERROR(ValidateFieldDescriptorType(field_descriptor,
...
absl::StatusOr<int64_t> GetInt64Field(const proto2::Message &message,
                                      const std::string &fieldname) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(message, fieldname));
  RETURN_IF_ERROR(ValidateFieldDescriptorType(field_descriptor,
...
absl::StatusOr<uint64_t> GetUint64Field(const proto2::Message &message,
                                        const std::string &fieldname) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(message, fieldname));
  RETURN_IF_ERROR(ValidateFieldDescriptorType(field_descriptor,
...
absl::StatusOr<std::string> GetStringField(const proto2::Message &message,
                                           const std::string &fieldname) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
                   GetFieldDescriptor(message, fieldname));
  RETURN_IF_ERROR(ValidateFieldDescriptorType(field_descriptor,
...
absl::Status SetBoolField(proto2::Message *message,
...
absl::Status SetInt32Field(proto2::Message *message,
...
absl::Status SetInt64Field(proto2::Message *message,
...
absl::Status SetUint64Field(proto2::Message *message,
...
absl::Status SetStringField(proto2::Message *message,
...
std::vector<std::string> GetAllFieldNames(const proto2::Message &message) {
...
absl::StatusOr<int> GetEnumField(const proto2::Message &message,
...
absl::Status SetEnumField(proto2::Message *message,
...
absl::StatusOr<p4::v1::TableEntry> PdTableEntryToPi(
    const p4::config::v1::P4Info &p4_info, const proto2::Message &pd) {
  ASSIGN_OR_RETURN(const auto info, CreateIrP4Info(p4_info));
...
absl::Status PiPacketInToPd(const IrP4Info &info,
                            ...
                            proto2::Message *pd_packet) {
  ASSIGN_OR_RETURN(const auto ir, PiPacketInToIr(info, pi_packet));
  return IrPacketInToPd(info, ir, pd_packet);
}

absl::StatusOr<p4::v1::PacketIn> PdPacketInToPi(const IrP4Info &info,
                                                const proto2::Message &packet) {
...
absl::StatusOr<p4::v1::PacketOut> PdPacketOutToPi(
    const IrP4Info &info, const proto2::Message &packet) {
...
absl::StatusOr<IrReadRequest> PdReadRequestToIr(
    const IrP4Info &info, const proto2::Message &read_request) {
...
absl::StatusOr<IrReadResponse> PdReadResponseToIr(
    const IrP4Info &info, const proto2::Message &read_response) {
...
absl::StatusOr<IrWriteRequest> PdWriteRequestToIr(
    const IrP4Info &info, const proto2::Message &write_request) {
...
// message.
static absl::Status IrActionInvocationToPd(const IrP4Info &ir_p4info,
                                           const IrActionInvocation &ir_action,
                                           proto2::Message *parent_message) {
  ASSIGN_OR_RETURN(
...
static absl::StatusOr<IrActionInvocation> PdActionInvocationToIr(
    ...
    const proto2::Message &pd_action) {
  ASSIGN_OR_RETURN(
      const auto &ir_action_info,
...
static absl::StatusOr<IrActionSetInvocation> PdActionSetToIr(
    const IrP4Info &ir_p4info, const proto2::Message &pd_action_set) {
...
absl::StatusOr<IrTableEntry> PdTableEntryToIr(const IrP4Info &ir_p4info,
                                              const proto2::Message &pd) {
  IrTableEntry ir;
...
absl::StatusOr<T> PdPacketIoToIr(const IrP4Info &info, const std::string &kind,
  ...
  proto2::Map<std::string, IrPacketIoMetadataDefinition> metadata_by_name;
  if (kind == "packet-in") {
...
absl::Status IrPacketIoToPd(const IrP4Info &info, const std::string &kind,
                            const T &packet, proto2::Message *pd_packet) {
  ASSIGN_OR_RETURN(auto *field_descriptor,
...
absl::StatusOr<IrPacketIn> PdPacketInToIr(const IrP4Info &info,
                                          const proto2::Message &packet) {
  return PdPacketIoToIr<IrPacketIn>(info, "packet-in", packet);
}
absl::StatusOr<IrPacketOut> PdPacketOutToIr(const IrP4Info &info,
                                            const proto2::Message &packet) {
...
static absl::Status IrUpdateStatusToPd(const IrUpdateStatus &ir_update_status,
                                       proto2::Message *pd_update_status) {
  RETURN_IF_ERROR(
...
static absl::Status IrWriteResponseToPd(
    ...
    proto2::Message *pd_rpc_response) {
