...
#include "third_party/p4lang_p4runtime/proto/p4/config/v1/p4types.proto.h"
...
absl::Status ProcessPacketIoMetadataDefinition(
    ...
    proto2::Map<uint32_t, IrPacketIoMetadataDefinition> *by_id,
    proto2::Map<std::string, IrPacketIoMetadataDefinition> *by_name,
    const P4TypeInfo &type_info) {
...
StatusOr<uint32_t> GetNumberInAnnotation(
    const proto2::RepeatedPtrField<std::string> &annotations,
...
StatusOr<IrActionInvocation> PiActionToIr(
    ...
    const proto2::RepeatedPtrField<IrActionReference> &valid_actions) {
...
StatusOr<O> PiPacketIoToIr(const IrP4Info &info, const std::string &kind,
  ...
  proto2::Map<uint32_t, IrPacketIoMetadataDefinition> metadata_by_id;
  if (kind == "packet-in") {
...
static absl::StatusOr<grpc::Status> IrWriteResponseToGrpcStatus(
  ...
  for (int i = 0; i < ir_write_response.statuses_size(); i++) {
    const IrUpdateStatus &ir_update_status = ir_write_response.statuses(i);
